...
const benefits = [
  { icon: '🎁', text: '入会赠饮1瓶（老款果酒3选1，自提免运费）' },
  { icon: '🎫', text: '每月1元小酒票（3种老款酒3选1，当月不领失效不累加）' },
  { icon: '⚡', text: '每周特价9.9元（老款果酒）' },
    ...
    <View className="fixed inset-0 z-50 flex items-center justify-center" style={{ backgroundColor: 'rgba(0,0,0,0.6)' }}>
      ...
      <View className="relative w-11/12 max-w-sm rounded-3xl overflow-hidden" style={{ backgroundColor: '#1E293B' }}>
        ...
        <View className="px-5 pb-4">
          {benefits.map((item, i) => (
            <View key={i} className="flex items-center gap-3 py-2">
        ...
        <View className="p-5 pt-2">
          <View className="rounded-xl overflow-hidden" style={{ background: 'linear-gradient(135deg, #F59E0B, #FBBF24)' }}>
            <Button className="w-full border-0" style={{ background: 'transparent', boxShadow: 'none' }} onClick={handleJoin} disabled={loading}>
              <Text className="text-gray-900 font-semibold">{loading ? '支付中...' : '立即开通 ¥9.9'}</Text>
            </Button>
          </View>
